...
class WebSocketManager {
  ...
  async handleAuth(ws, token) {
    try {
      ...
      console.log('Current connected clients:', Array.from(this.clients.keys()));
  ...
  async handleChatMessage(ws, data) {
    ...
    const message = {
      ...
      if (receiverWs) {
        console.log('Receiver is online, sending message directly');
      ...
      // Send confirmation back to sender with saved message details
      this.sendToClient(ws, {
  ...
  handleDisconnect(ws) {
    if (ws.userId) {
      ...
      console.log('Remaining connected users:', Array.from(this.clients.keys()));
    }
  }

  broadcastUserStatus(userId, isOnline) {
    console.log('Broadcasting user status:', { userId, isOnline });
    const statusMessage = {
      type: 'userStatus',
      userId,
  ...
  sendSuccess(ws, type) {
    console.log('Sending success:', type);
    this.sendToClient(ws, {
      type,
      status: 'success'
    });
